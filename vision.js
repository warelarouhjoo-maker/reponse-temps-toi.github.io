...
const ENDPOINT = "https://formspree.io/f/mjgpjpke";
...
// ── Envoi vers Formspree (supporte CORS, accepte JSON) ────────
async function sendData(payload) {
  try {
    const res = await fetch(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...payload,
        _subject: `Revelaté — ${payload.type || "réponse"}`,
    ...
    if (res.ok) {
      ...
      console.error("[Revelaté] Formspree erreur :", err);
      showToast("⚠ erreur envoi");
    }
  } catch (err) {
    console.error("[Revelaté] erreur réseau :", err);
